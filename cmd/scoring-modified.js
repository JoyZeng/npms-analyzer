...
const prepare = require('../lib/scoring_modified/prepare');
const aggregate = require('../lib/scoring_modified/aggregate');
const score_modified = require('../lib/scoring_modified/score');
const finalize = require('../lib/scoring_modified/finalize');
const bootstrap = require('./util/bootstrap');
const stats = require('./util/stats');

const log = logger.child({ module: 'cli/scoring-modified' });
...
function waitRemaining(delay, esClient) {
    ...
    return Promise.resolve(esClient.indices.getAlias({ name: 'npms-modified-current' }))
    .then((response) => {
        const index = Object.keys(response)[0];
        const timestamp = Number(index.replace(/^npms-modified-/, ''));
...
function cycle(delay, npmsNano, esClient) {
    ...
    .tap(() => {
        ...
        .then((aggregation) => aggregation && score_modified.all(aggregation, npmsNano, esClient));  // If aggregation is null, there were no evaluations
    })
    // Finalize
...
exports.handler = (argv) => {
    ...
    process.title = 'npms-analyzer-scoring-modified';
    logger.level = argv.logLevel || 'warn';

    // Bootstrap dependencies on external services
    bootstrap(['couchdbNpmsModified', 'elasticsearch'], { wait: true })
