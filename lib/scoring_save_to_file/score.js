...
const log = logger.child({ module: 'scoring_save_to_file/score' });
...
function calculateScore(value, aggregation, avgY) {
    ...
    // const MINIMUM_BEZIER_NORM_AVG = 0.0001;
    ...
    /*
     * Math becomes complicated when we mix up approximations and bezier curves.
     *
     * When the normalized average is too low we may not calculate the correct
     * score, so we will fallback to piecewise linear function.
     */

    // if (normAvg <= MINIMUM_BEZIER_NORM_AVG) {
    //     return calculateLinearScore(normValue, normAvg, avgY);
    // }

    return calculateBezierScore(value, aggregation, normValue, normAvg, avgY);
}

/**
 * Calculates the score according to a piecewise linear function, with the
 * following points: (0,0), (normValue, avgY) and (1, 1).
 *
 *
 * @param {number} normValue   The normalized value
 * @param {object} normAvg     The normalized average
 * @param {number} avgY        The avgY value to use for the intermediate point
 *
 * @return {number} The score
 */
// function calculateLinearScore(normValue, normAvg, avgY) {
//     const calculate = (x1, y1, x2, y2, x3) => (y1 * (x2 - x3) + y2 * (x3 - x1)) / (x2 - x1);
//
//     if (normValue < normAvg) {
//         return calculate(0, 0, normAvg, avgY, normValue);
//     }
//
//     return calculate(normAvg, avgY, 1, 1, normValue);
// }

/**
 * Calculates the score according to a bezier curbe, with the
 * following points: (0,0), (normValue, avgY), (normValue, avgY) and (1, 1)
 *
 *
 ...
 * @return {number} The score
 */
function calculateBezierScore(value, aggregation, normValue, normAvg, avgY) {
 ...
 * @return {Promise} The promise that fulfills when done
 */
function score(analysis, aggregation) {
    const name = analysis.collected.metadata.name;

    const finalScore = buildScore(analysis, aggregation);
    log.debug({ finalScore }, `Score of ${name} completed`);
    return finalScore;
}
